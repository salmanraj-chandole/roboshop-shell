...
SCHEMA_SETUP() {
  ...
  if [ "$schema_setup" == "mysql"  ]; then
    ...
    PRINT Load Schema
    mysql -h mysql.dev.salmanrajb80.online -uroot -pRoboShop@1 < /app/db/schema.sql &>>$LOG_FILE
    STAT $?

    PRINT Load Master Data
    mysql -h mysql.dev.salmanrajb80.online -uroot -pRoboShop@1 < /app/db/master-data.sql &>>$LOG_FILE
    STAT $?

    PRINT Create App Users
    mysql -h mysql.dev.salmanrajb80.online -uroot -pRoboShop@1 < /app/db/app-user.sql &>>$LOG_FILE
    STAT $?
  fi
